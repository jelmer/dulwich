...
class BranchPatternFilterTests(PorcelainTestCase):
    """Tests for branch pattern filtering functionality."""

    def setUp(self):
        super().setUp()
        # Build a commit graph with multiple commits
        [c1, c2, c3, c4, c5] = build_commit_graph(
            self.repo.object_store,
            [
                [1],  # c1
                [2, 1],  # c2 (master)
                [3, 2],  # c3 (develop, release)
                [4, 1],  # c4 (feature-auth)
                [5, 1],  # c5 (feature-ui)
            ],
        )
        ...
        self.repo.refs[b"refs/heads/master"] = c2.id
        self.repo.refs[b"refs/heads/develop"] = c3.id
        self.repo.refs[b"refs/heads/feature-auth"] = c4.id
        self.repo.refs[b"refs/heads/feature-ui"] = c5.id
        self.repo.refs[b"refs/heads/hotfix-123"] = c1.id
        self.repo.refs[b"refs/heads/hotfix-456"] = c2.id
        self.repo.refs[b"refs/heads/release-v1.0"] = c3.id

    def test_branch_list_no_pattern(self):
        """Test branch listing without pattern returns all branches."""
        branches = porcelain.branch_list(self.repo, pattern=None)
        branch_names = [b.decode() for b in branches]
        ...
        self.assertEqual(len(expected), len(branch_names))
        for branch in expected:
            self.assertIn(branch, branch_names)

    def test_branch_list_wildcard_pattern(self):
        """Test branch listing with wildcard patterns."""
        # Test feature-* pattern
        branches = list(porcelain.branch_list(self.repo, pattern="feature-*"))
        branch_names = [b.decode() for b in branches]
        expected = ["feature-auth", "feature-ui"]
        expected.sort()
        branch_names.sort()
        self.assertEqual(expected, branch_names)

        # Test hotfix-* pattern
        branches = list(porcelain.branch_list(self.repo, pattern="hotfix-*"))
        branch_names = [b.decode() for b in branches]
        expected = ["hotfix-123", "hotfix-456"]
        expected.sort()
        branch_names.sort()
        self.assertEqual(expected, branch_names)

    def test_branch_list_exact_match(self):
        """Test branch listing with exact match pattern."""
        branches = list(porcelain.branch_list(self.repo, pattern="master"))
        self.assertEqual([b"master"], branches)

        branches = list(porcelain.branch_list(self.repo, pattern="develop"))
        self.assertEqual([b"develop"], branches)

    def test_branch_list_no_matches(self):
        """Test branch listing with pattern that matches nothing."""
        branches = list(porcelain.branch_list(self.repo, pattern="nonexistent"))
        self.assertEqual([], branches)

        branches = list(porcelain.branch_list(self.repo, pattern="bug-*"))
        self.assertEqual([], branches)

    def test_branch_list_complex_patterns(self):
        """Test branch listing with complex shell patterns."""
        # Pattern with numbers
        branches = list(porcelain.branch_list(self.repo, pattern="*123*"))
        self.assertEqual([b"hotfix-123"], branches)

        # Pattern with question mark (single character)
        branches = list(porcelain.branch_list(self.repo, pattern="hotfix-??3"))
        self.assertEqual([b"hotfix-123"], branches)

        # Pattern matching anything containing 'v'
        branches = list(porcelain.branch_list(self.repo, pattern="*v*"))
        branch_names = [b.decode() for b in branches]
        self.assertIn("develop", branch_names)
        self.assertIn("release-v1.0", branch_names)

    def test_merged_branches_with_pattern(self):
        """Test merged branches filtering with patterns."""
        # First merge feature-ui into master
        self.repo.refs[b"refs/heads/master"] = self.repo.refs[b"refs/heads/feature-ui"]

        # Test pattern filtering on merged branches
        merged = list(porcelain.merged_branches(self.repo, pattern="feature-*"))
        branch_names = [b.decode() for b in merged]
        # Should find feature-ui as it's now merged, but not feature-auth
        self.assertIn("feature-ui", branch_names)

    def test_no_merged_branches_with_pattern(self):
        """Test unmerged branches filtering with patterns."""
        unmerged = list(porcelain.no_merged_branches(self.repo, pattern="feature-*"))
        branch_names = [b.decode() for b in unmerged]
        # Both feature branches should be unmerged initially
        expected = ["feature-auth", "feature-ui"]
        for branch in expected:
            self.assertIn(branch, branch_names)

    def test_branches_containing_with_pattern(self):
        """Test branches containing commit with pattern filtering."""
        # Get commit that's in multiple branches
        c3_sha = self.repo.refs[b"refs/heads/feature-ui"].decode()

        branches = list(
            porcelain.branches_containing(self.repo, c3_sha, pattern="*ui*")
        )
        branch_names = [b.decode() for b in branches]
        # Should only match feature-ui, not master even if c3 is in master's history
        expected_matches = ["feature-ui"]
        for branch in expected_matches:
            self.assertIn(branch, branch_names)

    def test_pattern_case_sensitivity(self):
        """Test that patterns are case-sensitive."""
        branches = list(porcelain.branch_list(self.repo, pattern="MASTER"))
        self.assertEqual([], branches)  # No match for uppercase

        branches = list(porcelain.branch_list(self.repo, pattern="master"))
        self.assertEqual([b"master"], branches)  # Exact case match

    def test_empty_pattern(self):
        """Test behavior with empty pattern."""
        branches = list(porcelain.branch_list(self.repo, pattern=""))
        self.assertEqual([], branches)  # Empty pattern matches nothing

    def test_branch_remotes_list_with_pattern(self):
        """Test remote branches filtering with patterns."""
        # Add some remote refs
        self.repo.refs[b"refs/remotes/origin/master"] = self.repo.refs[
            b"refs/heads/master"
        ]
        self.repo.refs[b"refs/remotes/origin/feature-new"] = self.repo.refs[
            b"refs/heads/feature-auth"
        ]
        self.repo.refs[b"refs/remotes/upstream/main"] = self.repo.refs[
            b"refs/heads/master"
        ]

        # Test pattern filtering on remote branches
        remotes = list(porcelain.branch_remotes_list(self.repo, pattern="origin/*"))
        remote_names = [b.decode() for b in remotes]

        expected = ["origin/master", "origin/feature-new"]
        for branch in expected:
            self.assertIn(branch, remote_names)

        # Should not match upstream/main
        self.assertNotIn("upstream/main", remote_names)

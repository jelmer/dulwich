...
def symbolic_ref(repo, ref_name, force=False, outstream=sys.stdout,
                 errstream=sys.stderr):
    """
    ...
    if ref_name:
        ...
        if not force and ref_path not in repo_obj.refs.keys():
            errstream.write('fatal: ref `%s` is not a ref' % ref_name)
            return

        repo_obj.refs.set_symbolic_ref('HEAD', ref_path)
        outstream.write('new symbolic ref is: %s' % ref_name)

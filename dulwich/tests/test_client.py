...
class GitClientTests(TestCase):
    ...
    def test_caps(self):
        agent_cap = (
            "agent=dulwich/%d.%d.%d" %
            dulwich.__version__).encode("ascii")
        self.assertEqual(
            set(
                [
                    b"multi_ack",
...
class ReportStatusParserTests(TestCase):
    ...
    def test_update_refs_error(self):
        ...
        parser.handle_packet(None)
        self.assertEqual([(b"refs/foo/bar", "need to pull")],
                         list(parser.check()))
...
class LocalGitClientTests(TestCase):
    ...
    def test_fetch_pack_none(self):
        ...
        ret = c.fetch_pack(
            s.path,
            lambda heads,
            **kwargs: [b"a90fa2d900a17e99b433217e988c4eb4a2e9a097"],
            graph_walker=walker,
            pack_data=out.write,
        )
        self.assertEqual({b"HEAD": b"refs/heads/master"}, ret.symrefs)
...
class HttpGitClientTests(TestCase):
    ...
    def test_url_redirect_location(self):
        ...
        for base_url in test_data.keys():
            ...
            resp = c.pool_manager.request(
                "GET", base_url + tail, redirect=False)
            ...
            if redirect_location:
                ...
                self.assertEqual(processed_url,
                                 redirect_location[: -len(tail)])
            else:
...
class DefaultUrllib3ManagerTest(TestCase):
    def test_no_config(self):
        ...
        self.assertEqual(
            manager.connection_pool_kw["cert_reqs"],
            "CERT_REQUIRED")
...
class RsyncUrlTests(TestCase):
    def test_simple(self):
        self.assertEqual(parse_rsync_url("foo:bar/path"),
                         (None, "foo", "bar/path"))
        self.assertEqual(
...
class CheckWantsTests(TestCase):
    ...
    def test_annotated(self):
        self.assertRaises(InvalidWants,
                          check_wants,
                          [b"2f3dc7a53fb752a6961d3a56683df46d4d3bf262"],
                          {b"refs/heads/blah": b"3f3dc7a53fb752a6961d3a56683df46d4d3bf262",
                           b"refs/heads/blah^{}": b"2f3dc7a53fb752a6961d3a56683df46d4d3bf262",
...
class GitCredentialStoreTests(TestCase):
    ...
    def test_nonmatching_scheme(self):
        self.assertEqual(
            get_credentials_from_store(
                b"http", b"example.org", fnames=[
                    self.fname]), None, )

    def test_nonmatching_hostname(self):
        self.assertEqual(
            get_credentials_from_store(
                b"https", b"noentry.org", fnames=[
                    self.fname]), None, )

    def test_match_without_username(self):
        self.assertEqual(
            get_credentials_from_store(
                b"https", b"example.org", fnames=[
                    self.fname]), (b"user", b"pass"), )

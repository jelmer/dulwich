...
class TestStringEncoding(unittest.TestCase):
    ...
    def test_encode_decode_string(self):
        ...
        test_strings = [b"", b"a", b"hello", b"refs/heads/master", b"a" * 1000]
        for s in test_strings:
            encoded = encode_string(s)
            stream = BytesIO(encoded)
            decoded = decode_string(stream)
...
class TestReftableRefsContainer(unittest.TestCase):
    ...
    def test_symbolic_ref(self):
        ...
        # Check symbolic ref resolves
        self.assertEqual(sha, self.container.read_loose_ref(b"HEAD"))

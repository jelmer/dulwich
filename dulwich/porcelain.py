...
def status(repo=".", ignored=False, untracked_files="all"):
    ...
    Args:
      ...
      untracked_files: How to handle untracked files:
            - "no": Show no untracked directories
                  (like `git status -uno`).
            - "all": show all files in untracked directories
                  (like `git status -uall`, dulwich default).
            - "normal": shows untracked files and directories.
                  (like `git status -unormal`, git default).
    ...
    with open_repo_closing(repo) as r:
        ...
        if untracked_files == "no":
            ...
            if untracked_files == "normal":
                walk = False
            elif untracked_files == "all":
                walk = True
            else:
                raise ValueError("untracked_files must be one of no, normal or all.")
            untracked_paths = get_untracked_paths(
                r.path, r.path, index, exclude_ignored=not ignored, walk=walk
            )
    ...
    def prune_dirnames(dirpath, dirnames):
        for i in range(len(dirnames) - 1, -1, -1):
            ...
            if ignore_manager.is_ignored(ip):
                ...
                if not any(
                    index_path.startswith(dirnames[i])
                    for index_path in map(os.fsdecode, index)
                ):
